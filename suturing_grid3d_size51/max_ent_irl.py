...
class MaxEntIRL:
    ...
    def __init__(self, trajectory_length, grid_size, n_trajectories):
        # super(MaxEntIRL, self).__init__(11)
        self.trajectory_length = trajectory_length
        self.grid_size = grid_size
        self.traj_index = random.randint(0, n_trajectories-1)

    # Calculates the reward function weights using the Max Entropy Algorithm
    def max_ent_irl(self, trajectories, discount, epochs, learning_rate):

        # filename = "/home/vignesh/Desktop/weights_grid11.txt"
        filename = "/home/vvarier/dvrk_automated_suturing/max_ent_trial4_parallel_compute_grid_size/iros2020/suturing_grid3d_size51/grid50_parallel/weights_grid50.txt"
        ...
        for i in range(epochs):
            ...
            total_states = len(trajectories[int(self.traj_index)])
            # total_states indicates that the last value of that trajectory data should be used as terminal state
            terminal_state_val_from_trajectory = trajectories[int(self.traj_index)][total_states - 1]
            ...
            optimal_policy, expected_svf = self.find_expected_svf(discount, total_states, trajectories, reward)
    ...
    def find_feature_expectations(self, feat_map, trajectories):
        ...
        for trajectory in trajectories:
            ...
            for state_value in trajectory:
                ...
                # print "state val index is ", state_value_index
    ...
    def find_expected_svf(self, discount, total_states, trajectories, reward):
        ...
        # 0 indicates that the first trajectory data is being used
        # total_states indicates that the last value of that trajectory data should be used as terminal state
        terminal_state_val_from_trajectory = state_values_from_trajectory[self.traj_index][total_states-1]
    ...
    def get_state_val_index(self, state_val):
        index_val = abs((state_val[0] + 0.04)/0.001 * pow(self.grid_size, 2)) + \
                    abs((state_val[1] - 0.020)/0.001 * pow(self.grid_size, 1)) + \
                    abs((state_val[2] + 0.142)/0.001)
