...
class MaxEntIRL:
    ...
    def max_ent_irl(self, trajectories, discount, n_trajectories, epochs, learning_rate):
        ...
        # filename = "/home/vvarier/dvrk_automated_suturing/iros2020/suturing_grid3d_size11_v_2/grid11_parallel/weights_grid11.txt"
        filename = "/home/vignesh/PycharmProjects/motion_planning_max_entropy_irl/suturing_grid3d_size11_v_2/grid11_parallel/weights_grid11.txt"
        ...
        feat_map = self.create_featmap_func(type_of_feat="coord", terminal_val=terminal_state_val_from_trajectory,
                                            env_obj=env_obj)
        print "feat map is ", feat_map.shape, feat_map
        ...
        for i in range(epochs):
            ...
            expected_svf = self.find_expected_svf(env_obj, discount, total_states, trajectories, reward)
            print "expected svf ", expected_svf
            ...
            t_complete_func.toc()
            val_func_time = t_complete_func.tocvalue()
            file_open.write(str(val_func_time))
            file_open.write("\n \n \n \n")
            file_open.close()
            # print "weights is ", weights
    ...
    def find_expected_svf(self, env_obj, discount, total_states, trajectories, reward):
    ...
    def featmap_func(self, i, env_obj):
        f = np.zeros(env_obj.n_states)
        arr_val = env_obj.states[i]
        for i_val in env_obj.lin_space_limits_x:
            for j_val in env_obj.lin_space_limits_y:
                for k_val in env_obj.lin_space_limits_z:
                    dist = abs(arr_val[0] - i_val) + abs(arr_val[1] - j_val) + abs(arr_val[2] - k_val)
                    f[self.get_state_val_index(np.array([i_val, j_val, k_val]))] = dist
        return f



    def create_featmap_func(self, type_of_feat, terminal_val, env_obj):
        if type_of_feat == "coord":
            features = []
            for n in range(env_obj.n_states):
                f = self.featmap_func(n, env_obj)
                features.append(f)

            return np.array(features)
            # return np.array(features).reshape((self.grid_size, self.grid_size, self.grid_size), order='F')
        else:
            # Default identity
            return np.eye(self.grid_size ** 3)
