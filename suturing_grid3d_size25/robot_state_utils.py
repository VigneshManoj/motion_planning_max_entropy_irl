...
class RobotStateUtils(concurrent.futures.ThreadPoolExecutor):
    def __init__(self, grid_size, discount, terminal_state_val_from_trajectory):
        ...
        self.lin_space_limits_x = np.linspace(-0.036, 0.012, self.grid_size, dtype='float32')
        self.lin_space_limits_y = np.linspace(0.020, 0.068, self.grid_size, dtype='float32')
        self.lin_space_limits_z = np.linspace(-0.148, -0.1, self.grid_size, dtype='float32')
    ...
    def create_action_set_func(self):
        ...
        action_set = []
        for pos_x in [-0.002, 0, 0.002]:
            for pos_y in [-0.002, 0, 0.002]:
                for pos_z in [-0.002, 0, 0.002]:
    ...
    # Add minimum joint value to the state and divide by the action value
...
if __name__ == '__main__':
    ...
    env_obj = RobotStateUtils(25, 0.9, goal)
    states = env_obj.create_state_space_model_func()
    action = env_obj.create_action_set_func()
    print "State space created is ", states
    print "size of states ", len(states[100])
    print "actions is ", action
    index_val = env_obj.get_state_val_index(states[530])
